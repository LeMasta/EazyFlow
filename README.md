# EazyFlow

面向项目工作的 Windows 11 桌面工具：用时间表回答“还有多少工作”，用项目页集中保存任务文件、参考文件、交付文件和其他文件。

## Beta 功能

- 小时表：横轴为工作时间、纵轴为项目；未定结束和逾期项目随当前时间推进，支持缩放时间轴。
- 工作日历：支持双休、单休、大小周、自定义工作周，以及中国法定节假日、调休和额外休息日。
- B1 月历：有交付时间的项目显示为连续色条；未定交付项目使用渐隐色条和“持续中 / 未定交付”标记。
- 月历日期格支持独立滚动，双击日期可直接进入当天小时表。
- 项目库：集中查看全部项目与关联项目组；项目支持按名称、状态、交付时间筛选及多种时间顺序排序。
- 关联项目组：新建或编辑项目时可关联一个更早的项目，首次关联会创建并命名项目组；组详情用链式视图呈现各阶段，并支持编辑组名、说明和识别色。
- 智能配色：新建项目或项目组默认优先选择当前尚未使用的色板颜色，减少日程中的颜色冲突。
- 剪贴板文件流：文件区支持 Ctrl+V 导入资源管理器文件、文件夹和截图；选中文件后可用 Ctrl+C 复制回资源管理器。
- 项目工作区：每个项目使用项目名建立独立目录，仅在分类中实际存入内容时创建对应子目录。
- 文件操作：可选择或拖放文件及整个文件夹；悬停显示重命名、打开、定位和删除操作，并自动同步资源管理器内的改名、新增与删除。
- 独立图片查看器：双击图片打开类 QQ 的独立预览窗口，支持缩放、旋转、适应窗口、1:1 和拖拽查看。
- Windows 文件剪贴板：在项目内选中文件按 Ctrl+C，可直接粘贴到资源管理器、桌面及支持文件粘贴的外部程序。
- 存储设置：可在设置页调整项目文件存储位置，现有项目会随之安全迁移。
- 最近项目按最近访问时间显示，默认三天，可在设置页调整范围。
- 项目状态随开始时间自动变化；计划交付与实际结束分开记录，可回改结束时间并比较提前或逾期。
- GitHub Releases 自动检查与下载安装更新。

## 本地开发

需要 Node.js 22 或更高版本。

```bash
npm install
npm run dev
```

构建 Windows 安装程序：

```bash
npm run dist:win
```

## 发布更新

更新 `package.json` 中的版本号并推送相同版本的标签，例如 `v0.1.0`。GitHub Actions 会生成 NSIS 安装程序和自动更新所需的 `latest.yml`。

```bash
git tag v0.1.0
git push origin v0.1.0
```

项目文件默认保存在 Electron 的 `userData/workspace/projects` 目录内，可在应用设置页更改；项目元数据仍保存在 `userData/workspace`，Windows 通常对应 `%APPDATA%/EazyFlow/workspace`。
