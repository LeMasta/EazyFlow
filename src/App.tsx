...
const defaultSchedule:WorkSettings={startHour:9,endHour:18,breakStart:12,breakEnd:13,weekPreset:'双休',workDays:[1,2,3,4,5],bigWeekStartsThisWeek:true,publicHolidays:true,makeupWorkdays:true,irregularRest:false,restDates:[]}
const projectEnd=(p:Project,fallback:Date)=>p.completedAt?parseISO(p.completedAt):p.dueAt?parseISO(p.dueAt):fallback
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>{active?<ProjectPage project={active} onBack={()=>setActiveId(null)} onChanged={load} onDelete={()=>remove(active)}/>:<>
      ...
      <section className="toolbar"><div className="segmented"><button className={view==='hour'?'active':''} onClick={()=>setView('hour')}><Clock3/>小时表</button><button className={view==='month'?'active':''} onClick={()=>setView('month')}><LayoutGrid/>月历</button></div>{view==='hour'&&<button className="schedule-button" onClick={()=>setScheduleOpen(true)}><Settings2/>工作日历</button>}</section>
      {settings&&(view==='hour'?<HourView date={cursor} projects={visible} settings={settings} onOpen={p=>setActiveId(p.id)} onContext={showContext}/>:<MonthView month={cursor} projects={visible} settings={settings} onOpen={p=>setActiveId(p.id)} onContext={showContext}/>)}
    </>}</main>
...
function HourView({date,projects,settings,onOpen,onContext}:{date:Date;projects:Project[];settings:WorkSettings;onOpen:(p:Project)=>void;onContext:(e:React.MouseEvent,p:Project)=>void}){
  ...
  const shown=projects.filter(p=>parseISO(p.startAt)<=end&&projectEnd(p,end)>=start)
...
function HourRow({project,date,settings,zoom,timelineWidth,onOpen,onContext}:{project:Project;date:Date;settings:WorkSettings;zoom:number;timelineWidth:number;onOpen:()=>void;onContext:(e:React.MouseEvent)=>void}){
  const start=parseISO(project.startAt),due=project.dueAt?parseISO(project.dueAt):null,actualEnd=project.completedAt?parseISO(project.completedAt):due
  ...
  return <div className={`horizontal-row ${completed?'completed-row':''}`} onContextMenu={onContext}><button className="row-project" onDoubleClick={onOpen}><i style={{background:completed?'#7b9a82':project.color}}/><span><b>{project.name}</b><small>{completed?'✓ 已完成':effectiveStatus(project)}</small></span></button><div className="row-timeline" style={{width:timelineWidth,backgroundSize:`${zoom}px 100%`}}><div className="break-band" style={{left:(settings.breakStart-settings.startHour)*zoom,width:(settings.breakEnd-settings.breakStart)*zoom}}/><button className={`horizontal-bubble ${!actualEnd?'open-ended':''} ${completed?'completed-bubble':''}`} style={{left:(a-settings.startHour)*zoom,width:Math.max(34,(b-a)*zoom),'--bubble':project.color} as React.CSSProperties} onDoubleClick={onOpen} title={`${project.name}\n${format(start,'M月d日 HH:mm')} 开始\n${completed&&project.completedAt?'实际完成 '+format(parseISO(project.completedAt),'M月d日 HH:mm'):due?format(due,'M月d日 HH:mm')+' 交付':'交付日期未定'}\n双击进入，右键管理`}><b>{completed?'✓ ':''}{project.name}</b><small>{hourText(a)}{actualEnd?`—${hourText(b)}`:'—持续中'}</small></button>{showNow&&!completed&&<div className="now-line" style={{left:(now.getHours()+now.getMinutes()/60-settings.startHour)*zoom}}/>}</div></div>
}

function MonthView({month,projects,settings,onOpen,onContext}:{month:Date;projects:Project[];settings:WorkSettings;onOpen:(p:Project)=>void;onContext:(e:React.MouseEvent,p:Project)=>void}){
  ...
  const onDay=(p:Project,d:Date)=>isWithinInterval(d,{start:startOfDay(parseISO(p.startAt)),end:endOfDay(projectEnd(p,end))})
  return <section className="month-card"><div className="weekdays">{'一二三四五六日'.split('').map(d=><span key={d}>周{d}</span>)}</div><div className="month-grid">{days.map(day=>{const all=projects.filter(p=>onDay(p,day)),working=isWorkingDate(day,settings),festival=settings.publicHolidays?getFestival(dateKey(day)):'';return <div className={`day ${!isSameMonth(day,month)?'muted':''} ${isSameDay(day,new Date())?'today-day':''} ${working?'':'rest-day'}`} key={dateKey(day)}><span className="day-num">{format(day,'d')}</span>{festival&&<small className="festival">{festival}</small>}<div className="day-items">{all.slice(0,3).map(p=>{const begins=isSameDay(day,parseISO(p.startAt)),completed=p.status==='已完成';return <button key={p.id} onDoubleClick={()=>onOpen(p)} onContextMenu={e=>onContext(e,p)} className={`month-pill ${!p.dueAt?'undated':''} ${completed?'completed-pill':''}`} style={{'--bubble':p.color} as React.CSSProperties} title={`${p.name}\n${p.description}\n${completed&&p.completedAt?'实际完成 '+format(parseISO(p.completedAt),'M月d日 HH:mm'):p.dueAt?'交付 '+format(parseISO(p.dueAt),'M月d日 HH:mm'):'交付日期未定'}`}><span>{completed?'✓ ':''}{begins?p.name:`↪ ${p.name}`}</span>{!p.dueAt&&!completed&&<em>{begins?'持续中':'未定交付'}</em>}</button>})}{all.length>3&&<small className="more">+ {all.length-3} 个</small>}</div></div>})}</div></section>
}
...
function ProjectEditor({project,onClose,onSave}:{project:Project;onClose:()=>void;onSave:(patch:Partial<Project>)=>void}){
  const [name,setName]=useState(project.name),[description,setDescription]=useState(project.description),[start,setStart]=useState(toLocalInput(project.startAt)),[due,setDue]=useState(toLocalInput(project.dueAt)),[color,setColor]=useState(project.color),[status,setStatus]=useState<ProjectStatus>(project.status)
  const valid=Boolean(name.trim()&&start&&(!due||new Date(due)>new Date(start)))
  return <div className="modal-backdrop"><form className="modal editor-modal" onSubmit={e=>{e.preventDefault();if(!valid)return;const completedAt=status==='已完成'?(project.completedAt||new Date().toISOString()):undefined;onSave({name:name.trim(),description:description.trim(),startAt:new Date(start).toISOString(),dueAt:due?new Date(due).toISOString():undefined,color,status,completedAt})}}><div className="modal-head"><div><p className="eyebrow">项目信息</p><h2>项目编辑</h2></div><button type="button" onClick={onClose}><X/></button></div><label>项目名称<input autoFocus value={name} onChange={e=>setName(e.target.value)}/></label><label>项目说明<textarea value={description} onChange={e=>setDescription(e.target.value)}/></label><div className="form-row"><label>开始时间<input type="datetime-local" value={start} onChange={e=>setStart(e.target.value)}/></label><label>计划交付 <small>可不填</small><input type="datetime-local" value={due} min={start} onChange={e=>setDue(e.target.value)}/></label></div><label>项目状态<select value={status} onChange={e=>setStatus(e.target.value as ProjectStatus)}><option value="进行中">进行中</option><option value="已完成">已完成</option><option value="已归档">已归档</option></select><small className="field-help">未到开始时间时，时间表会自动显示为“未开始”；无需手动选择。</small></label><label>识别颜色<div className="color-row expanded">{colors.map(c=><button type="button" className={color===c?'picked':''} style={{background:c}} key={c} onClick={()=>setColor(c)}/>)}<span className="custom-color"><input type="color" value={color} onChange={e=>setColor(e.target.value)}/><em>自定义</em></span></div></label><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>取消</button><button className="primary" disabled={!valid}>保存更改</button></div></form></div>
}
