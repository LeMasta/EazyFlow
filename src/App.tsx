...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav><button className={!active&&!settingsPage?'selected':''} onClick={()=>{setActiveId(null);setSettingsPage(false)}}><CalendarDays/>时间表</button><div className="nav-caption">最近项目</div>{projects.slice(0,6).map(p=><button key={p.id} className={active?.id===p.id&&!settingsPage?'selected':''} onClick={()=>{setActiveId(p.id);setSettingsPage(false)}}><i style={{background:p.color}}/>{p.name}</button>)}</nav>
      <div className="sidebar-bottom"><button className={settingsPage?'selected':''} onClick={()=>{setActiveId(null);setSettingsPage(true)}}><Settings2/>设置</button>{update&&<div className="update-note">{update}</div>}{update.includes('已下载')&&<button style={{background:'#f7f5ef',color:'#24231f'}} onClick={()=>window.eazyflow.installUpdate()}><RefreshCw/>立即安装更新</button>}<button onClick={()=>window.eazyflow.checkForUpdates()}><RefreshCw/>检查更新</button></div>
    </aside>
    <main>{settingsPage&&settings?<SettingsPage storageRoot={storageRoot} settings={settings} onSchedule={()=>setScheduleOpen(true)} onStorageChanged={load}/>:active?<ProjectPage project={active} now={now} onBack={()=>setActiveId(null)} onChanged={load} onDelete={()=>remove(active)}/>:<>
      ...
      <section className="toolbar"><div className="segmented"><button className={view==='hour'?'active':''} onClick={()=>setView('hour')}><Clock3/>小时表</button><button className={view==='month'?'active':''} onClick={()=>setView('month')}><LayoutGrid/>月历</button></div></section>
...
function SettingsPage({storageRoot,settings,onSchedule,onStorageChanged}:{storageRoot:string;settings:WorkSettings;onSchedule:()=>void;onStorageChanged:()=>void}){
  ...
  return <div className="settings-page"><header className="settings-header"><p className="eyebrow">EazyFlow</p><h1>设置</h1><p>集中管理项目文件和工作日历。</p></header><section className="settings-card"><div className="settings-card-icon"><FolderOpen/></div><div className="settings-card-body"><h2>项目文件存储</h2><p>每个项目使用项目名建立文件夹，并包含“任务文件、参考文件、交付文件、其他”四个子文件夹。</p><div className="storage-path" title={storageRoot}>{storageRoot}</div><small>更改位置时，现有项目会先完整复制到新位置，再切换数据库记录；原目录随后移入回收站。</small></div><div className="settings-card-actions"><button className="secondary" onClick={()=>window.eazyflow.revealStorageRoot()}><ExternalLink/>打开位置</button><button className="primary" onClick={changeStorage}>更改位置</button></div></section><section className="settings-card"><div className="settings-card-icon"><CalendarDays/></div><div className="settings-card-body"><h2>工作日历</h2><p>{settings.weekPreset} · {hourText(settings.startHour)}—{hourText(settings.endHour)} · 休息 {hourText(settings.breakStart)}—{hourText(settings.breakEnd)}</p><small>工作日、法定节假日、请假、调休和加班会实际参与项目延伸计算。</small></div><div className="settings-card-actions"><button className="primary" onClick={onSchedule}>编辑工作日历</button></div></section></div>
}
...
function ProjectFiles({project,onChanged}:{project:Project;onChanged:()=>void}){
  ...
  return <section className="materials-workspace"><div className="material-tabs">{categories.map(c=>{const Icon=c.icon,count=project.files.filter(f=>f.category===c.id).length;return <button key={c.id} className={active===c.id?'active':''} onClick={()=>setActive(c.id)}><Icon/><span>{c.label}</span><em>{count}</em></button>})}</div><FileSection projectId={project.id} category={category} files={files} onImport={()=>run(()=>window.eazyflow.importFiles(project.id,category.id))} onImportFolder={()=>run(()=>window.eazyflow.importFolder(project.id,category.id))} onDrop={dropped=>run(()=>window.eazyflow.importDroppedFiles(project.id,category.id,dropped))} onOpen={f=>window.eazyflow.openFile(project.id,f.id).catch(error=>alert(error.message))} onReveal={f=>window.eazyflow.revealFile(project.id,f.id).catch(error=>alert(error.message))} onDelete={async f=>{if(confirm(`确定删除“${f.name}”吗？\n\n${f.kind==='folder'?'整个文件夹及其内容':'文件'}会移入 Windows 回收站。`))await run(()=>window.eazyflow.deleteFile(project.id,f.id))}}/></section>
}

const imageExtensions=new Set(['.png','.jpg','.jpeg','.gif','.webp','.bmp','.svg'])
function FileSection({projectId,category,files,onImport,onImportFolder,onDrop,onOpen,onReveal,onDelete}:{projectId:string;category:typeof categories[number];files:ProjectFile[];onImport:()=>void;onImportFolder:()=>void;onDrop:(files:File[])=>void;onOpen:(f:ProjectFile)=>void;onReveal:(f:ProjectFile)=>void;onDelete:(f:ProjectFile)=>void}){
  const Icon=category.icon,[dragging,setDragging]=useState(false),images=files.filter(f=>f.kind!=='folder'&&imageExtensions.has(f.extension.toLowerCase())),documents=files.filter(f=>f.kind==='folder'||!imageExtensions.has(f.extension.toLowerCase()))
  return <section className={`file-section material-panel ${dragging?'dragging':''}`} onDragEnter={e=>{e.preventDefault();setDragging(true)}} onDragOver={e=>e.preventDefault()} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget as Node))setDragging(false)}} onDrop={e=>{e.preventDefault();setDragging(false);const dropped=Array.from(e.dataTransfer.files);if(dropped.length)onDrop(dropped)}}><div className="file-heading"><div className="file-heading-icon"><Icon/></div><div><h3>{category.label}<span>{files.length}</span></h3><p>{category.hint}；图片可直接预览</p></div><div className="file-heading-actions"><button onClick={onImport}><Plus/>添加文件</button><button onClick={onImportFolder}><FolderOpen/>添加文件夹</button></div></div>{files.length===0?<div className="drop-hint material-empty"><Upload/><b>{dragging?'松开即可复制到项目':'把文件、文件夹、截图或图片拖到这里'}</b><span>也可以使用上方按钮选择，内容会完整复制一份保存在项目内</span></div>:<><div className="compact-drop"><Upload/>可继续把文件或整个文件夹拖放到此区域</div>{images.length>0&&<div className="image-gallery">{images.map(f=><article className="image-card" key={f.id} onDoubleClick={()=>onOpen(f)} title="双击打开原图"><img src={window.eazyflow.filePreviewUrl(projectId,f.id)} alt={f.name}/><div className="image-caption"><span><b>{f.name}</b><small>{bytes(f.size)}</small></span><div className="file-actions"><button title="打开文件" onClick={()=>onOpen(f)}><ExternalLink/></button><button title="打开所在位置" onClick={()=>onReveal(f)}><FolderOpen/></button><button className="danger" title="移入回收站" onClick={()=>onDelete(f)}><Trash2/></button></div></div></article>)}</div>}{documents.length>0&&<div className="file-list document-list">{documents.map(f=><div className={`file-row ${f.kind==='folder'?'folder-row':''}`} key={f.id} onDoubleClick={()=>onOpen(f)} title={`双击打开${f.kind==='folder'?'文件夹':'文件'}`}><div className="file-type">{f.kind==='folder'?<FolderOpen/>:f.extension.replace('.','').slice(0,4)||'FILE'}</div><div className="file-info"><b>{f.name}</b><small>{f.kind==='folder'?'文件夹 · 双击打开':`${bytes(f.size)} · 双击打开`}</small></div><div className="file-actions"><button title={`打开${f.kind==='folder'?'文件夹':'文件'}`} onClick={()=>onOpen(f)}><ExternalLink/></button><button title="打开所在位置" onClick={()=>onReveal(f)}><FolderOpen/></button><button className="danger" title="移入回收站" onClick={()=>onDelete(f)}><Trash2/></button></div></div>)}</div>}</>}{dragging&&<div className="drop-overlay"><Upload/>复制到“{category.label}”</div>}</section>
}
