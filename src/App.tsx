...
function HourRow({project,date,now,settings,zoom,timelineWidth,onOpen,onContext}:{project:Project;date:Date;now:Date;settings:WorkSettings;zoom:number;timelineWidth:number;onOpen:()=>void;onContext:(e:React.MouseEvent)=>void}){
  ...
  return <div className={`horizontal-row ${completed?'completed-row':''} ${overdue?'overdue-row':''} ${project.predecessorId?'continued-row':''}`} onContextMenu={onContext}>
    <button className="row-project" onDoubleClick={onOpen}><i style={{background:overdue?'#d0444f':project.color}}/><span><span className="row-title-line"><b>{project.name}</b>{project.predecessorId&&<ProjectTag/>}</span><small>{completed?'✓ 已完成':overdue?'! 已逾期':effectiveStatus(project,now)}</small></span></button>
    <div className="row-timeline" style={{width:timelineWidth,backgroundSize:`${zoom}px 100%`}}><div className="break-band" style={{left:(settings.breakStart-settings.startHour)*zoom,width:(settings.breakEnd-settings.breakStart)*zoom}}/><button className={`horizontal-bubble ${followsNow?'open-ended':''} ${completed?'completed-bubble':''} ${overdue?'overdue-bubble':''} ${project.predecessorId?'continued-bubble has-project-tags':''}`} style={{left:(a-settings.startHour)*zoom,width:Math.max(34,(b-a)*zoom),'--bubble':project.color} as React.CSSProperties} onDoubleClick={onOpen} title={`${project.name}\n${project.predecessorId?'延续项目\n':''}${format(start,'M月d日 HH:mm')} 开始\n${completed&&project.completedAt?'实际结束 '+format(parseISO(project.completedAt),'M月d日 HH:mm'):overdue&&due?'已超过计划交付 '+format(due,'M月d日 HH:mm'):due?format(due,'M月d日 HH:mm')+' 计划交付':'结束时间未定'}\n双击进入，右键管理`}><b>{completed?'✓ ':overdue?'! ':''}{project.name}</b><small>{hourText(a)}—{hourText(b)}{followsNow?' · 随当前时间推进':''}</small>{project.predecessorId&&<span className="bubble-tag-rail"><ProjectTag attached/></span>}</button>{showNow&&!completed&&<div className="now-line" style={{left:(now.getHours()+now.getMinutes()/60-settings.startHour)*zoom}}/>}</div>
  </div>
}
...
function ProjectTag({compact=false,attached=false}:{compact?:boolean;attached?:boolean}){return <span className={`project-tag continuation-tag ${compact?'compact':''} ${attached?'attached':''}`}><Link2/><span>延续</span></span>}
