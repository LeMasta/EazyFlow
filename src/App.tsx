...
const projectEnd=(p:Project)=>p.completedAt?parseISO(p.completedAt):p.dueAt?parseISO(p.dueAt):endOfDay(new Date())
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main>{active?<ProjectPage project={active} onBack={()=>setActiveId(null)} onChanged={load} onDelete={()=>remove(active)}/>:<>
      ...
      {settings&&(view==='hour'?<HourView date={cursor} projects={visible} settings={settings} onOpen={p=>setActiveId(p.id)} onContext={showContext}/>:<MonthView month={cursor} projects={visible} settings={settings} onOpen={p=>setActiveId(p.id)} onContext={showContext} onDateContext={(e,date)=>{e.preventDefault();setDayContext({x:e.clientX,y:e.clientY,date})}}/>)}
    </>}</main>
...
function HourView({date,projects,settings,onOpen,onContext}:{date:Date;projects:Project[];settings:WorkSettings;onOpen:(p:Project)=>void;onContext:(e:React.MouseEvent,p:Project)=>void}){
  ...
  const shown=working?projects.filter(p=>parseISO(p.startAt)<=end&&projectEnd(p)>=start):[]
...
function MonthView({month,projects,settings,onOpen,onContext,onDateContext}:{month:Date;projects:Project[];settings:WorkSettings;onOpen:(p:Project)=>void;onContext:(e:React.MouseEvent,p:Project)=>void;onDateContext:(e:React.MouseEvent,date:Date)=>void}){
  ...
  const onDay=(p:Project,d:Date)=>{
    ...
    const projectStart=startOfDay(parseISO(p.startAt)),projectFinish=endOfDay(projectEnd(p))
    return projectStart<=projectFinish&&isWithinInterval(d,{start:projectStart,end:projectFinish})
  }
  return <section className="month-card"><div className="weekdays">{'一二三四五六日'.split('').map(d=><span key={d}>周{d}</span>)}</div><div className="month-grid">{days.map(day=>{const all=projects.filter(p=>onDay(p,day)),working=isWorkingDate(day,settings),festival=settings.publicHolidays?getFestival(dateKey(day)):'',override=settings.dayOverrides?.[dateKey(day)];return <div onContextMenu={e=>onDateContext(e,day)} className={`day ${!isSameMonth(day,month)?'muted':''} ${isSameDay(day,new Date())?'today-day':''} ${working?'':'rest-day'} ${override?`override-${override}`:''}`} key={dateKey(day)}><span className="day-num">{format(day,'d')}</span><small className="day-kind">{override==='leave'?'请假':override==='timeoff'?'调休':override==='overtime'?'加班':festival|| (working?'工作日':'休息')}</small><div className="day-items">{all.slice(0,3).map(p=>{const begins=isSameDay(day,parseISO(p.startAt)),completed=p.status==='已完成',last=isSameDay(day,projectEnd(p));return <button key={p.id} onDoubleClick={()=>onOpen(p)} onContextMenu={e=>{e.stopPropagation();onContext(e,p)}} className={`month-pill ${!p.dueAt?'undated':''} ${completed?'completed-pill':''}`} style={{'--bubble':p.color} as React.CSSProperties} title={`${p.name}\n${p.description}\n${completed&&p.completedAt?'实际完成 '+format(parseISO(p.completedAt),'M月d日 HH:mm'):p.dueAt?'交付 '+format(parseISO(p.dueAt),'M月d日 HH:mm'):'交付日期未定'}`}><span>{completed&&last?'✓ ':''}{begins?p.name:`↪ ${p.name}`}</span>{!p.dueAt&&!completed&&<em>{last?'今日延伸':'持续中'}</em>}</button>})}{all.length>3&&<small className="more">+ {all.length-3} 个</small>}</div></div>})}</div></section>
}
